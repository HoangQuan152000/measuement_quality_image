...
{
public:
    explicit BatteryInspectorAnode(float const &pxSize, BatteryInspectorPole const &pole, BatteryInspectorJR const &jr)
        : PixelRef{pxSize}, mIspPole{std::cref(pole)}, mIspJR{std::cref(jr)}
    {
    }
    explicit BatteryInspectorAnode(float &&pxSize) = delete;
