...
{
public:
    explicit BatteryInspectorCathode(float const &pxSize, BatteryInspectorPole const &pole, BatteryInspectorJR const &jr)
        : PixelRef{pxSize}, mIspPole{std::cref(pole)}, mIspJR{std::cref(jr)}
    {
    }

    explicit BatteryInspectorCathode(float &&pxSize) = delete;
