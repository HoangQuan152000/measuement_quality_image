...
{
public:
    explicit BatteryInspectorBeading(float const &pxSize) : PixelRef{pxSize}
    {
    }
    explicit BatteryInspectorBeading(float &&pxSize) = delete;

    auto Inspect(cv::Mat const &src, const std::vector<cv::Point> &contour) const -> BatteryInspectorBeadingResult;
